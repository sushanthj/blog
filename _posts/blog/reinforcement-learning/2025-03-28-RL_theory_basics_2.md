...
In the [previous post](/blog/reinforcement-learning/basics_1/), we trained a policy by imitating expert demonstrations. The training loop was straightforward: collect expert data, compute a gradient that pushes the policy toward every expert action, update the parameters, repeat.
...
```mermaid
flowchart TB
    subgraph LOOP["The shared training loop — imitation learning and policy gradients both follow it"]
        direction LR
        C1["1 · Collect trajectories"]
        C2["2 · Compute gradient<br/>∇ log π<sub>θ</sub>(a|s)"]
        C3["3 · Update θ<br/>θ ← θ + α∇J"]
        C1 --> C2 --> C3
        C3 -. "repeat" .-> C1
    end
    ...
    subgraph IL["Imitation Learning"]
        direction TB
        ILD["<b>Data:</b> expert demonstrations"]
        ILG["<b>Gradient:</b> ∇ log π(a|s) <b>× 1</b><br/><i>all actions weighted equally</i>"]
        ILD ~~~ ILG
    end

    subgraph PG["Policy Gradients"]
        direction TB
        PGD["<b>Data:</b> agent's own experience"]
        PGG["<b>Gradient:</b> ∇ log π(a|s) <b>× R(τ)</b><br/><i>weighted by reward</i>"]
        PGD ~~~ PGG
    end
    ...
    classDef collect fill:#1a1d27,stroke:#e8c88a,color:#e8c88a;
    classDef grad fill:#1a1d27,stroke:#6c9eff,color:#e0e0e6;
    classDef upd fill:#17301f,stroke:#2e8b57,color:#e0e0e6;
    classDef card fill:#1a1d27,stroke:#2a2d3a,color:#e0e0e6;
    class C1 collect;
    class C2 grad;
    class C3 upd;
    class ILD,ILG,PGD,PGG card;
```

<div style="text-align:center;color:#8b8fa3;font-size:13px;margin:-0.5em 0 2em">Same loop, two differences: where the data comes from, and how the gradient is weighted.</div>
...
```mermaid
flowchart LR
    A["<b>1 · Run policy π<sub>θ</sub></b><br/><i>collect a batch of trajectories</i>"]
    B["<b>2 · Estimate ∇J(θ)</b><br/>3 · Update θ ← θ + α∇J"]

    A -- "trajectories" --> B
    B -- "repeat with improved policy" --> A

    classDef run fill:#1a1d27,stroke:#6c9eff,color:#e0e0e6;
    classDef upd fill:#301a22,stroke:#f38ba8,color:#e0e0e6;
    class A run;
    class B upd;
```
...
<video width="100%" autoplay loop muted playsinline>
  <source src="/images/blog/reinforcement-learning/RL_theory_basics_2/video_pg_reward_weight.mp4" type="video/mp4">
</video>
...
![REINFORCE vs IL gradient](/images/blog/reinforcement-learning/RL_theory_basics_2/svg3_reinforce_vs_il.svg)
...
![Humanoid walking trajectories](/images/blog/reinforcement-learning/RL_theory_basics_2/svg4_humanoid_trajectories.svg)
...
<video width="100%" autoplay loop muted playsinline>
  <source src="/images/blog/reinforcement-learning/RL_theory_basics_2/video_pg_rtg_comparison.mp4" type="video/mp4">
</video>
...
![Baseline example](/images/blog/reinforcement-learning/RL_theory_basics_2/svg5_baseline.svg)
...
```mermaid
flowchart LR
    A["<b>Collect data</b><br/>states, actions, rewards<br/><i>using policy θ₁</i>"]
    B["<b>loss.backward()</b><br/>θ₁ → θ₂"]
    C["<b>Now what?</b><br/>Data came from θ₁, but we now have θ₂.<br/>Throw it away and recollect?<br/><i>Or reuse it with importance weights?</i>"]

    A --> B --> C

    classDef collect fill:#1a1d27,stroke:#e8a87c,color:#e0e0e6;
    classDef grad fill:#17301f,stroke:#2e8b57,color:#e0e0e6;
    classDef prob fill:#301a22,stroke:#f38ba8,color:#e0e0e6;
    class A collect;
    class B grad;
    class C prob;
```

<div style="text-align:center;color:#8b8fa3;font-size:13px;margin:-0.5em 0 2em">On-policy = recollect every time (wasteful). Off-policy = reuse with correction (importance sampling).</div>
...
![Importance weights rebalance](/images/blog/reinforcement-learning/RL_theory_basics_2/svg8_importance_weights.svg)
